...
// HTML and API responses must stay fresh. Only content-hashed bundles are immutable.
export function staticCacheControl(pathname) {
  pathname = publicAssetPath(pathname);
  if (/^\/src\/(?:chunks\/)?[\w-]+-[A-Z0-9]{8}\.(?:js|css)$/.test(pathname)) {
    return "public, max-age=31536000, immutable";
  }
  if (/^\/assets\/.+\.(?:svg|png|webp|jpe?g|ico)$/.test(pathname)) {
    return "public, max-age=3600";
  }
  return "no-cache";
}

// This virtual URL keeps image responses inside the Worker on asset-first hosts.
export function publicAssetPath(pathname) {
  return pathname.startsWith("/media/") ? pathname.replace(/^\/media\//, "/assets/") : pathname;
}
