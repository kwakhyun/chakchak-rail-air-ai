...
import { defaultTargetDateTime } from "./http-security.mjs";
...
const fusionCaches = new WeakMap();
const MAX_FUSION_CACHE_ENTRIES = 128;
const tagoCircuitByFetch = new WeakMap();

function tagoCircuitFor(fetchImpl, date) {
  let byDate = tagoCircuitByFetch.get(fetchImpl);
  if (!byDate) { byDate = new Map(); tagoCircuitByFetch.set(fetchImpl, byDate); }
  ...
  if (!state) {
    ...
    if (byDate.size >= 16) byDate.delete(byDate.keys().next().value);
...
function sourceResult(definition, mode, summary, data = null, reasonCode = null, extra = {}) {
  return {
    ...definition,
    mode: mode === "live" && (!data || (Array.isArray(data) && !data.length)) ? "unavailable" : mode,
    summary,
...
export function normalizeFlight(payload, flightId) {
  ...
  const match = items.find((item) => cleanText(item.flightId || item.flight_id).replace(/\s/g, "").toUpperCase() === wanted) ;
...
export function normalizeAirportRail(payload) {
  return itemsFrom(payload).slice(0, 20).map((item) => compactObject({
    ...
    stationName: cleanText(item.stnNm || item.stationName, "", 40),
    departureStation: cleanText(item.depStnNm || item.departureStation, "", 40),
    arrivalStation: cleanText(item.arrStnNm || item.arrivalStation, "", 40),
...
function closestWeather(payload, targetDateTime) {
  ...
  if (bestDistance > 90 * 60_000) return null;
  return {
...
async function fetchTrains({ key, date, fetchImpl, retryDelayMs = TAGO_RETRY_DELAY_MS, korailEnabled = false }) {
  ...
  const circuit = tagoCircuitFor(fetchImpl, date);
  const now = Date.now();
...
export function publicDataStatus(env = process.env) {
  const circuits = tagoCircuitByFetch.get(fetch);
  const tagoCircuit = circuits ? [...circuits.values()].at(-1) : null;
  const now = Date.now();
  return {
...
export async function buildDataFusion(query = {}, options = {}) {
  ...
  const defaultDateTime = defaultTargetDateTime(options.now instanceof Date ? options.now : new Date());
  const targetDateTime = cleanText(query.targetDateTime, defaultDateTime, 40);
  const date = /^\d{4}-\d{2}-\d{2}/.test(targetDateTime) ? targetDateTime.slice(0, 10) : defaultDateTime.slice(0, 10);
  if (!Number.isFinite(Date.parse(targetDateTime))) throw new TypeError("INVALID_TARGET_DATETIME");
  const key = env.DATA_GO_KR_API_KEY || "";
  const tourismKey = env.TOUR_API_KEY || key;
  const fetchImpl = options.fetchImpl || fetch;

  let fusionCache = fusionCaches.get(fetchImpl);
  if (!fusionCache) { fusionCache = new Map(); fusionCaches.set(fetchImpl, fusionCache); }
  // Scope credentials without retaining them in cache keys.
  const cacheKey = `${flightId}:${targetDateTime}:${Boolean(key)}:${Boolean(tourismKey)}:${env.KORAIL_OPEN_API_ENABLED === "true"}`;
  for (const [cacheId, entry] of fusionCache) {
    if (Date.now() - entry.createdAt >= 45_000) fusionCache.delete(cacheId);
  }
  ...
  if (options.cache !== false) {
    if (fusionCache.size >= MAX_FUSION_CACHE_ENTRIES) fusionCache.delete(fusionCache.keys().next().value);
    fusionCache.set(cacheKey, { createdAt: Date.now(), value });
  }
  return value;
}
