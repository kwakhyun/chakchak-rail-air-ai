...
**로컬 수집 파이프라인 준비 완료·공개 접수와 실측 성능 주장 차단.** 2026-08-30 기준 실제 완료 여정은 0건이다. 공개 Sites 배포판은 등록·관측 API를 열지 않는다. 따라서 모델 카드의 시뮬레이션 지표를 실제 운영 정확도로 해석하지 않으며 Brier, P50/P90 실측 오차, 탑승 성공률을 공개하지 않는다.
...
현재 모델은 P95도 출력하지만 이 프로토콜과 저장 스키마는 P50/P90만 평가한다. P95는 저장 필드, MAE와 포함률 계산, 품질 테스트를 추가하기 전까지 실측 성능 범위에 포함하지 않는다.
...
P2-B 현장 운영 테스트 6개는 일회용 참여코드, 모집 단계, 동의문 해시, 운영 기한초과, 선택 동의 내보내기, 위변조 탐지, 철회 삭제와 상태 전환을 검증한다. HTTP 보안 테스트를 포함한 전체 Node 테스트는 57개이며, Chromium 핵심 흐름 2개를 별도로 확인한다. `calculationSelfCheck`의 40건은 계산식 정답만 확인하는 합성 고정자료이며 실제 운영 성능에 포함하지 않는다. 현장 운영 절차는 `docs/field-pilot-operations.md`를 따른다.

## 다음 실증 순서

1. 서비스 고도화 과정에서는 공개 접수를 열지 않고, 먼저 화면 이해도와 철회 절차를 비식별 사용성 점검으로 확인한다.
2. 운영기관 협의, 관리형 데이터베이스, HTTPS 쓰기 API와 개인정보 보호 절차를 갖춘 뒤 소규모 자발적 참여를 시작한다.
3. 완료 30건 전에는 스키마·현장 버튼·완결률·계산 오류만 확인하며 모델 성능 결론을 내리지 않는다.
4. 자료가 충분히 쌓이면 시간대·이동지원·복합위험 구간을 분리해 방향성을 검토한다. 30·100·300건은 통계적 성능 보장이 아니라 재산정이 필요한 운영 게이트다.
5. 코레일·인천공항공사와 승강장 도착 및 탑승 성공의 기준시각을 합의하고, 기관 대조가 가능한 시점에 사전등록 분석계획과 검정력 분석을 고정한다.
