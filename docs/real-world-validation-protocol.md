...
P2-B 현장 운영 테스트 6개는 일회용 참여코드, 모집 단계, 동의문 해시, 운영 기한초과, 선택 동의 내보내기, 위변조 탐지, 철회 삭제와 상태 전환을 검증한다. 전체 제품 테스트는 51개다. `calculationSelfCheck`의 40건은 계산식 정답만 확인하는 합성 고정자료이며 실제 운영 성능에 포함하지 않는다. 현장 운영 절차는 `docs/field-pilot-operations.md`를 따른다.
