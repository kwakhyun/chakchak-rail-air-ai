...
flowchart LR
  ...
  D[TourAPI·시드 일정] --> N
  N --> S[실시간 여정 신호 파생]
  S --> CAI[착착 Connection AI 도메인 기준선 + 단조 P50·P90·확률 3-head]
  S --> M[Seeded Monte Carlo 1,200회 안전 검증]
  CAI --> P[AI P50·P90·탑승확률·additive Shapley]
  M --> G[안전 가드레일]
  P --> G
  G --> O[P1-B 안전·도착·대기·접근성 다목적 선택]
  O --> T[관광 일정 제약 재계획]
...
`scripts/train_chakchak_model.mjs`는 정상·복합위험·저위험·이동지원 조건을 층화해 학습 2,400개, 안전 보정 200개, 분리 검증 800개를 생성합니다. 각 조건의 정답은 독립 시드 3개에서 총 1,200회 시뮬레이션한 평균입니다. 시간의 도메인 기준선 위에 불확실성 기반 단조 P50/P90 헤드를 학습하고, P90은 별도 보정 세트의 오차 97.5분위만큼 안전 보정합니다. 탑승확률은 불확실성을 감산한 보수적 환승 여유에 단조 등위회귀를 적용합니다. 가중치·특성 계약·품질지표는 `src/chakchak-model-data.js`에 버전 고정 아티팩트로 저장됩니다.

- 입력: 항공지연, 날씨·입국 혼잡, 수하물, 이동지원·큰 짐, 열차까지 남은 시간과 도메인 결합 특성
- 출력: 플랫폼 도착 P50/P90, 후보별 탑승확률·위험등급, 안전·도착·대기·접근성 종합점수가 가장 높은 적격 후보
- 설명: 6개 조건의 exact Shapley 값을 계산해 `기준 확률 + 변화 합 = 현재 확률`인 워터폴과 검증오차 기반 안정성 범위를 생성
- 현재 층화 분리 검증 800개: P50 MAE 1.678분, 안전보정 P90 MAE 4.901분, P90 과소예측 3.5%, probability MAE 0.0268
- 복합위험 검증 200개: P90 MAE 4.764분, 과소예측 9.0%. 구버전 44.763분 대비 약 89.4% 개선
- 독립 제품 감사 400개: 전체 P90 MAE 4.647분, 복합위험 P90 MAE 4.803분, 3,000개 단조 비교 위반 0건
- 적용: 모델과 1,200회 엔진이 모두 안전 기준을 통과한 후보 중 안전 55%·목적지 도착 20%·불필요 대기 15%·접근성 10% 종합점수가 가장 높은 열차를 사용. 접근성 위반은 하드 가드레일로 제외

검증 지표는 시뮬레이터 정답에 대한 모델 근사 성능이며 실제 운영 정확도가 아닙니다. 그래서 모델 버전은 `1.1.0-sim`, `realWorldCalibrated: false`로 고정하고 UI의 지표 바로 옆에도 실측 운영 성능이 아님을 표시합니다. 항공지연 0~240분, 날씨·입국 혼잡 0~2단계, 수하물 추가지연 0~90분, 열차 여유 35~360분, 승차 버퍼 0~30분의 확인 범위를 벗어나면 API가 OOD 사유를 반환하고 1,200회 안전 엔진으로 자동 전환합니다.
...
- 실측 지표: 모델/보수결합 Brier·log loss·ECE, P50/P90 MAE, P90 포함률
...
- `POST /api/chakchak/predict`: 자체 모델 버전·지표, 후보별 P50/P90·확률·Shapley 워터폴·4축 의사결정 점수, 모델/엔진 불일치와 최종 추천을 반환
...
2. 실제 항공편 검색과 6종 데이터의 갱신주기·결측·쿼터·장애 관제
3. 코레일톡/공항철도/관광 예약 딥링크
4. P2 실제 완료 30건 파일럿과 100건 구간별 방향성 검증
