...
**현장 파일럿 소프트웨어 준비 완료·모집 시작 전.** 2026-08-04 기준 운영 단계는 `READY`, 발급 참여코드 0개, 등록 0건, 완료 0건이다. 실제 성능을 보여 주는 수치가 아니며, 관리형 데이터베이스와 기관 원장 협약 전에는 공개·다중 인스턴스 운영 준비 완료로 표현하지 않는다.
...
현재 저장소는 단일 Node 프로세스가 원자적으로 갱신하는 권한 `0600` 파일이다. 본선 현장의 한 대 운영 서버에서는 사용할 수 있지만 다음 경우에는 적합하지 않다.
...
공개 파일럿 전에는 관리형 PostgreSQL, HTTPS 호스팅, 비밀키 관리, 자동 백업을 연결해야 한다. 이 단계는 신규 외부 계정과 프로젝트 생성이 필요하므로 사용자 승인 후 진행한다.
...
전체 제품 테스트는 43개다. 실제 완료 표본은 현재 0건이다.
